...
struct LocalBridgeClient: Sendable {
    ...
    func fetchState() async throws -> BridgeState {
        ...
        request.setValue(LocalBridge.protocolVersion, forHTTPHeaderField: "X-Voice2028-Protocol")
    ...
    enum BridgeClientError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .invalidResponse:
                "Voice2028 returned an invalid response."
            case .http(let status):
                "Voice2028 communication failed (HTTP \(status))."
            }
        }
    }
}
