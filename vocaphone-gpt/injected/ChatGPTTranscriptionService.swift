...
struct ChatGPTTranscriptionService {
    ...
    func transcribe(
        ...
        let boundary = "VocaPhoneGPT-\(UUID().uuidString)"
        let multipartURL = try makeMultipartBodyFile(audioURL: audioURL, boundary: boundary)
        ...
        guard http.statusCode == 200 else {
            ...
            case 401, 403: throw TranscriptionError.authenticationExpired
            case 429: throw TranscriptionError.rateLimited
            default: throw TranscriptionError.http(http.statusCode, detail)
            }
        }
    ...
    private func makeMultipartBodyFile(audioURL: URL, boundary: String) throws -> URL {
        let bodyURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("vocaphone-gpt-upload-\(UUID().uuidString)")
