...
struct ChatGPTCleanupService {
    ...
    你是 VocaPhone-GPT 的语音转录文本整理器。你的唯一任务是整理用户提供的转录文本。
    ...
    func clean(
        ...
        request.setValue("VocaPhone-GPT", forHTTPHeaderField: "originator")
        request.setValue("VocaPhone-GPT/1.0 (iOS)", forHTTPHeaderField: "User-Agent")
        ...
        guard http.statusCode == 200 else {
            ...
            case 401, 403: throw CleanupError.authenticationExpired
            case 429: throw CleanupError.rateLimited
            default: throw CleanupError.http(http.statusCode, detail)
            }
        }
    ...
    enum CleanupError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .invalidResponse: "智能整理返回了无效响应。"
            case .authenticationExpired: "ChatGPT 登录已失效，请打开 VocaPhone 重新登录。"
            case .rateLimited: "ChatGPT 智能整理暂时达到频率限制。"
            case .http(let status, let detail):
                detail.isEmpty ? "智能整理失败（HTTP \(status)）。" : "智能整理失败（HTTP \(status)）：\(detail)"
            case .model(let message): "智能整理失败：\(message)"
            case .noText: "智能整理没有返回文本。"
            }
        }
    }
}
